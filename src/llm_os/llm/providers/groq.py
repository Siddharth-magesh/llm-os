...
Cloud LLM inference using Groq's ultra-fast API.
"""
...
class GroqProvider(BaseLLMProvider):
    ...
    Groq provides extremely fast inference for popular models like:
    - llama3-70b-8192
    - llama3-8b-8192
    - mixtral-8x7b-32768
    - gemma-7b-it
    ...
    def _convert_tools(
        ...
        """Convert tools to OpenAI format (Groq uses OpenAI-compatible API)."""
        result = []
        for tool in tools:
    ...
    async def stream(
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                raise ProviderUnavailableError(f"Groq streaming failed: {e}")
        except Exception as e:
            raise ProviderUnavailableError(f"Groq streaming failed: {e}")
