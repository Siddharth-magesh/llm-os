...
Access 400+ AI models through one unified API.
"""
...
class OpenRouterProvider(BaseLLMProvider):
    """
    OpenRouter provider for accessing 400+ AI models.
    ...
    def __init__(
        ...
        timeout: float = 120.0,
        app_name: str = "LLM-OS",
        app_url: str = "https://github.com/llm-os",
    ):
    ...
    def _convert_tools(
        ...
        """Convert tools to OpenAI format (OpenRouter uses OpenAI-compatible API)."""
        result = []
        for tool in tools:
    ...
    async def complete(
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                raise ProviderUnavailableError(f"OpenRouter request failed: {e}")
        except Exception as e:
            raise ProviderUnavailableError(f"OpenRouter request failed: {e}")
    ...
    async def stream(
        ...
        try:
            async with client.stream("POST", "/chat/completions", json=payload) as response:
                ...
                async for line in response.aiter_lines():
                    ...
                    if "tool_calls" in delta and delta["tool_calls"]:
                        # Note: Streaming tool calls are complex, simplified here
            ...
            if e.response.status_code == 401:
                ...
                raise ProviderUnavailableError(f"OpenRouter streaming failed: {e}")
        except Exception as e:
            raise ProviderUnavailableError(f"OpenRouter streaming failed: {e}")
